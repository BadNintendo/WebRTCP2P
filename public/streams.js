...
var onloaded = () => {
	...
	StickPM.member(window,document);
};
...
var onYouTubePlayerAPIReady = () => {
	...
	var yturlsrc = 'https://www.youtube-nocookie.com/embed/'+templink+'?autoplay='+setmediaautoplay+'&controls=2&iv_load_policy=3&modestbranding=0&rel=0&showinfo=1&enablejsapi=1&wmode=opaque&scheme=https&origin=https://StickPM.com';
	ytlive.src = yturlsrc;
	...
	if(string != null && string != undefined && string.length>=1){
		...
		if(text != null && text != undefined && text.length>=1){
			...
			if(text.includes('!room')){ var more = splitUp[2].toLowerCase(); if(more == undefined || more == '') more = null; else what = more.toLowerCase(); socket.emit('chat data', { type : 'room', action : action.toLowerCase(), type : type.toLowerCase(), what : what }); }
				...
				socket.emit('chat data', { type : 'test',  message : line });
			}
			else if(text.includes('!roll')) { amount = line.replace('dice', ''); StickPM.rolldice(amount); }
	...
	member : function (content,page){
		...
		createConnection = (data) => {
			...
			/*peerConnection.ontrack = function({ streams: [stream] }) {
				const newvar = stream;
				if (newvar.live != selectedRoom) addRemoteVideo(stream);
				newvar.live = selectedRoom;
			};*/
		...
		addRemoteVideo = (stream) => {
			...
			var third = document.createElement("div");
				...
				third.title = 'Close';
				third.onclick = (function() { if(openConnections[peerID]) openConnections[peerID].close(); removeElement('stream-ch-' + peerID); });
				third.innerHTML = '&nbsp;';
		...
		loaddata = () => {
			...
			page.getElementById('register').onclick = (x) => {
				getid('mainregister').style.display='flex';
			};
			page.getElementById('openlogin').onclick = (x) => {
				getid('mainlogin').style.display='flex';
			};
			...
			page.getElementById('StickPM_send').onclick = () => {
				sendData(getid('input').textContent);
			};
			...
			page.getElementById('local-close-stream').onclick = () => { 
				...
				socket.emit('close local', { StickPM : 'Coded by Chris' });
			};
